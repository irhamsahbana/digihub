...
type Config struct {
	...
	DigihubPostgres struct {
		Host     string `env:"DIGIHUB_POSTGRES_HOST" env-default:"localhost"`
		Port     string `env:"DIGIHUB_POSTGRES_PORT" env-default:"5432"`
		Username string `env:"DIGIHUB_POSTGRES_USER" env-default:"postgres"`
		Password string `env:"DIGIHUB_POSTGRES_PASSWORD" env-default:"postgres"`
		Database string `env:"DIGIHUB_POSTGRES_DB" env-default:"venatronics"`
		SslMode  string `env:"DIGIHUB_POSTGRES_SSL_MODE" env-default:"disable"`
	}
	Mail struct {
	...
	DigihubStorage struct {
		Key      string `env:"DIGIHUB_STORAGE_KEY"`
		Secret   string `env:"DIGIHUB_STORAGE_SECRET"`
		Endpoint string `env:"DIGIHUB_STORAGE_ENDPOINT"`
		Region   string `env:"DIGIHUB_STORAGE_REGION"`
		Bucket   string `env:"DIGIHUB_STORAGE_BUCKET"`
	}
	Oauth struct {
		Google struct {
